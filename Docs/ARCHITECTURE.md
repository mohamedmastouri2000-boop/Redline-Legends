...
Visual pass (all under `Scripts/Editor/Setup`, regenerated every run, no hand-authored art yet):

- `ProceduralTextures` — tileable asphalt (lane markings), kerb stripes + run-off, ground, concrete, building
  facade with a lit-window emission mask, kerb glow mask, and the UI sprites (rounded 9-slice, gauge ring,
  circle, chevron, pedal, menu fade, shade). Saved once under `Materials/Textures`; real textures replace 1:1.
- `CarMeshBuilder` — lofts a body per class (60 stations x 20-point rings): sills, wheel arches, shoulder line,
  side windows and sloping windscreen/rear glass as a second sub-mesh, paint roof; plus lights, splitter,
  diffuser, mirrors, wing and spoked wheels. `PlaceholderCarBuilder` wires it into the `*_visual` prefabs.
- `TerrainBuilder` — heightfield with a road-following flat band, blend to rolling hills, slope-split rock
  sub-mesh, one MeshCollider. `TrackDressing` places themed props on it; buildings are metric-UV boxes so one
  facade material tiles per 4 m bay on any building size.
- `SceneCapture` (`Redline Legends > Capture > Render all scenes`, or `SceneCapture.CaptureAllBatch` with
  `REDLINE_CAPTURE_DIR`, no `-nographics`) renders every scene plus menu/garage/HUD UI shots to PNG for review.
- `UiKit` — every canvas gets a `SafeArea` child (`SafeAreaFitter`); panels are rounded 9-slice by default and
  drop the sprite when stretched edge to edge; `CreateGauge` builds the 270-degree rpm dial, `CreatePad` the
  round icon touch controls. `MenuShowcase` spins the selected car on a lit stage behind the main menu.
...
- Visual pass (post Phase 6) — procedural textures, lofted car bodies, terrain + dressing, night city glow,
  rounded UI with radial gauge, icon pads, safe area, menu car showcase. Verified with `SceneCapture` renders.
